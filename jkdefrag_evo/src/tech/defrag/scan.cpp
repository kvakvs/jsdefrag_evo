...
void DefragRunner::calculate_zones(DefragState &data) {
    ...
    Clusters64 size_of_movable_files[3] = {};
    ...
    Clusters64 size_of_unmovable_fragments[3] = {};
    Clusters64 old_zone_end[3] = {};
    Clusters64 zone_end[3] = {};

    for (auto iterate = 1; iterate <= 10; iterate++) {
        // Calculate the end of the zones
        zone_end[0] = size_of_movable_files[0] + size_of_unmovable_fragments[0] +
                      Clusters64((uint64_t) (data.total_clusters_.value() * data.free_space_ / 100.0));
        ...
        // Show debug info
        gui->show_debug(DebugLevel::DetailedFileInfo, nullptr, std::format(
                L"Zone calculation, iteration {}: 0 - " NUM_FMT " - " NUM_FMT " - " NUM_FMT,
                iterate, zone_end[0].value(), zone_end[1].value(), zone_end[2].value()
        ));
        ...
        for (auto zone = 0; zone <= 2; zone++) size_of_unmovable_fragments[zone] = {};

        // The MFT reserved areas are counted as unmovable data
        for (auto &mft_exclude: data.mft_excludes_) {
            if (mft_exclude.start_ < zone_end[0]) {
                size_of_unmovable_fragments[0] += mft_exclude.end_ - mft_exclude.start_;
            } else if (mft_exclude.start_ < zone_end[1]) {
                size_of_unmovable_fragments[1] += mft_exclude.end_ - mft_exclude.start_;
            } else if (mft_exclude.start_ < zone_end[2]) {
                size_of_unmovable_fragments[2] += mft_exclude.end_ - mft_exclude.start_;
            }
        }
...
// For debugging only: compare the data with the output from the FSCTL_GET_RETRIEVAL_POINTERS function call. Note:
// Reparse points will usually be flagged as different. A reparse point is a symbolic link. The CreateFile call will
// resolve the symbolic link and retrieve the info from the real item, but the MFT contains the info from
// the symbolic link
[[maybe_unused]] void DefragRunner::compare_items([[maybe_unused]] DefragState &data, const FileNode *item) const {
    ...
    gui->show_debug(DebugLevel::AlwaysLog, nullptr, std::format(
            NUM_FMT L" {}",
            item->get_item_lcn().value(), item->get_long_fn()
    ));
    ...
    do {
        ...
        if (max_loop <= 0) {
            gui->show_always(L"  FSCTL_GET_RETRIEVAL_POINTERS error: Infinite loop");
        ...
        retrieve_param.StartingVcn.QuadPart = vcn.as<LONGLONG>();

        error_code = DeviceIoControl(file_handle, FSCTL_GET_RETRIEVAL_POINTERS,
                                     &retrieve_param, sizeof retrieve_param, &extent_data,
                                     sizeof extent_data, &bytes_returned, nullptr);
        ...
        for (i = 0; i < extent_data.extent_count_; i++) {
            ...
            if (fragment == item->fragments_.end()) {
                ...
                if (fragment->lcn_ != extent_data.extents_[i].lcn_) {
                    gui->show_debug(DebugLevel::AlwaysLog, nullptr, std::format(
                            L"  Different LCN in fragment: " NUM_FMT " <> " NUM_FMT,
                            fragment->lcn_.value(), extent_data.extents_[i].lcn_.value()
                    ));
                }

                if (fragment->next_vcn_ != extent_data.extents_[i].next_vcn_) {
                    gui->show_debug(DebugLevel::AlwaysLog, nullptr, std::format(
                            L"  Different NextVcn in fragment: " NUM_FMT " <> " NUM_FMT,
...
// Compare two items
// TODO: Move to FileNode class
CompareResult DefragRunner::compare_items(FileNode *item_1, FileNode *item_2, SortField sort_field) {
    int result;

    // If one of the items is nullptr then the other item is bigger
    if (item_1 == nullptr) return CompareResult::FirstIsSmaller;
    if (item_2 == nullptr) return CompareResult::FirstIsBigger;

    // Return zero if the items are exactly the same
    if (item_1 == item_2) return CompareResult::Equal;
    ...
    switch (sort_field) {
        case SortField::Name: {
            ...
            if (result != 0) return (CompareResult) result;
            break;
        }

        case SortField::Size: {
            if (item_1->bytes_ < item_2->bytes_) return CompareResult::FirstIsSmaller;
            if (item_1->bytes_ > item_2->bytes_) return CompareResult::FirstIsBigger;
            break;
        }

        case SortField::LastAccess: {
            if (item_1->last_access_time_ > item_2->last_access_time_) return CompareResult::FirstIsSmaller;
            if (item_1->last_access_time_ < item_2->last_access_time_) return CompareResult::FirstIsBigger;
            break;
        }

        case SortField::LastChange: {
            if (item_1->mft_change_time_ < item_2->mft_change_time_) return CompareResult::FirstIsSmaller;
            if (item_1->mft_change_time_ > item_2->mft_change_time_) return CompareResult::FirstIsBigger;
            break;
        }

        case SortField::Created: {
            if (item_1->creation_time_ < item_2->creation_time_) return CompareResult::FirstIsSmaller;
            if (item_1->creation_time_ > item_2->creation_time_) return CompareResult::FirstIsBigger;
            break;
        }
    }

    // The sort_field of the items is equal, so we must compare all the other fields to see if they are really equal.
    ...
    if (item1_lcn < item2_lcn) return CompareResult::FirstIsSmaller;
    if (item1_lcn > item2_lcn) return CompareResult::FirstIsBigger;
...
void DefragRunner::scan_dir(DefragState &data, const wchar_t *mask, FileNode *parent_directory) {
    ...
    do {
        ...
        item->bytes_ = Bytes64(find_file_data.nFileSizeHigh * ((uint64_t) MAXDWORD + 1) + find_file_data.nFileSizeLow);
        ...
        if (is_fragmented(item.get(), Clusters64(0), item->clusters_count_)) {
        ...
        if (item->clusters_count_.is_zero() || item->fragments_.empty()) continue;

        // Draw the item on the screen
        colorize_disk_item(data, item.get(), Clusters64(0), Clusters64(0), false);
